...
const favTrue = '../assets/images/yellow_star.png';
...
searchBar.addEventListener('keyup', (e)=> {
    ...
    if (searchString.length < 2){       // avoiding huge number of search results
        document.getElementById('results').innerHTML = 'Agrega por lo menos 3 caracteres';
    }
    else{
...
document.addEventListener('click', (event) => {
    ...
    // Favourite button
        ...
        if (favs.indexOf(id) != -1){
            ...
            customAlert('failure','Eliminado de favoritos');
        }
        else{
            favs.push(id);
            localStorage.setItem('superheroFavs',JSON.stringify(favs));
            event.target.src = favTrue;
            customAlert('success','Agregado a favoritos');
        }
    }
});
